...
```bash cURL
curl -X POST "https://platform.plaud.ai/api//files/upload-s3/generate-presigned-urls" \
...
```curl cURL
curl -X POST "https://platform.plaud.ai/api/files/upload-s3/complete-upload" \
  -H "Authorization: Bearer <your_api_token>" \
  -H "Content-Type": application/json" \
  -d '{
    "owner_id": "user_12345",
