...
icon: "/assets/icons/sidebar/device_operations.svg"
---
...
DEVICE_ID=$(curl -X POST "https://api.plaud.ai/api/devices/bind" \
...
```curl cURL
curl -X POST "https://api.plaud.ai/api/devices/unbind" \
  -H "Authorization: Bearer $PLAUD_API_TOKEN" \
  -H "Content-Type": "application/json" \
  -d '{
    "owner_id": "user_12345",
...
curl -X GET "https://api.plaud.ai/api/devices/$DEVICE_ID" \
  -H "Authorization: Bearer $PLAUD_API_TOKEN"
```
</CodeGroup>
